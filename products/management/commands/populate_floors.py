    ...
    def handle(self, *args, **options):
        ...
        if options['stream']:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f'Stream "{options["stream"]}" does not exist')
                )
                return
        else:
        ...
        if not streams:
            self.stdout.write(
                self.style.ERROR('No streams found. Please create streams first.')
            )
            return
        ...
        for stream in streams:
            ...
            for floor_name, description in floors:
                ...
                if created:
                    total_created += 1
                    self.stdout.write(
                        self.style.SUCCESS(f'  ✓ Created floor: {floor_name}')
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(f'  - Floor already exists: {floor_name}')
                    )
