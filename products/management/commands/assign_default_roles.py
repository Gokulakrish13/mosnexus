    ...
    def handle(self, *args, **options):
        ...
        for user in users:
            ...
            if created:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f'Created profile for {user.username} with role: {custom_profile.role}'
                    )
                )
            else:
                # Update existing profiles that don't have roles set
                if not custom_profile.role:
                    updated_count += 1
                    if user.is_superuser:
                        custom_profile.role = 'super_admin'
                    elif user.is_staff:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f'Updated profile for {user.username} with role: {custom_profile.role}'
                        )
                    )
